...
import { FileText, Search, Plus, SlidersHorizontal, Calendar, X } from 'lucide-react';
...
import { Report, ReportType, getReportTypeLabel , ReportTypes } from '@/models/report';
...
const ReportArchive: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(report => (
        ...
        (report.patientId && report.patientId.toLowerCase().includes(query)) ||
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div className="mb-4 md:mb-0">
              ...
              <p className="text-gray-600 mt-1">All your medical reports in one place</p>
            </div>
            <div>
              <Button onClick={() => navigate('/scan')} className="bg-medical hover:bg-medical-dark">
                    ...
                    <Button variant="outline" className="gap-1">
                      ...
                      {(selectedTypes.length > 0 || selectedDateRange !== 'all') && (
                        <Badge className="ml-1 bg-medical text-white">
                          {selectedTypes.length + (selectedDateRange !== 'all' ? 1 : 0)}
                        </Badge>
                      )}
                    </Button>
                  </PopoverTrigger>
  ...
  <div key={type} className="flex items-center space-x-2">
    <Checkbox 
      ...
      onCheckedChange={() => handleFilterTypeChange(type)}
    />
    <Label htmlFor={`filter-${type}`}>{getReportTypeLabel(type)}</Label>
  </div>
))}
              ...
              <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
                ...
                {reports.length > 0 ? (
                  ...
                  <>
                    ...
                    <p className="text-gray-500 mb-4">Start by scanning your first medical report</p>
                    <Button onClick={() => navigate('/scan')} className="bg-medical hover:bg-medical-dark">
                      <Plus className="h-4 w-4 mr-2" />
                      Add New Report
                    </Button>
