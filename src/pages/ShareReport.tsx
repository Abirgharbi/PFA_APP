...
const ShareReport: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await getDoctors(user.token);

        if (Array.isArray(response)) {
          setAvailableUsers(response);
          setFilteredUsers(response);
        } else {
          console.error("Expected an array of users, got:", response);
          setAvailableUsers([]);
          setFilteredUsers([]);
        }
      } catch (error) {
  ...
  const toggleVisibility = async () => {
    try {
      ...
      setReport((prev) => ({ ...prev!, isPublic: result.isPublic }));
      toast.success(
        `Le rapport est maintenant ${result.isPublic ? "public" : "privé"}.`
      );
    } catch (error) {
                ...
                <p className="text-sm text-gray-600">
                  ...
                  <span
                    className={`ml-2 font-semibold ${
                      report.isPublic ? "text-green-600" : "text-red-600"
                    }`}
                  >
              ...
              <Card>
                ...
                <CardFooter className="flex justify-between">
                  ...
                  <Button
                    ...
                    disabled={
                      isSharing ||
                      selectedUsers.length === (report.sharedWith?.length || 0)
                    }
                  >
                    {isSharing ? (
