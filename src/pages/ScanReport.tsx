...
import { Camera, FileText, Check, AlertTriangle, Loader2 } from "lucide-react";
...
const ScanReport: React.FC = () => {
  ...
  const [extractedData, setExtractedData] = useState<FormattedOCRData | null>(
    null
  );
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      const data: OCRResponse = await uploadImageForOCR(
        image as File,
        reportType
      );
      ...
      setTitle(
        `Analyse sanguine - ${formattedData.patientInfo.name || "Patient"}`
      );

      toast.success("Analyse terminée avec succès", {
        description: `Temps de traitement: ${data.processing_time.toFixed(
          2
        )} secondes`,
      });
    } catch (error) {
  ...
  const createReportFromExtractedData = (): any => {
    ...
    return {
      ...
      patientName:
        patientName || extractedData.patientInfo.name || "Patient Inconnu",
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="mb-6">
            ...
            <CardContent>
              <Tabs
                value={activeTab}
                onValueChange={(value) =>
                  setActiveTab(value as "capture" | "review")
                }
              >
                ...
                <TabsContent value="capture" className="pt-4">
                  <div className="mb-6">
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="Analyse sanguine">
                          Analyse sanguine
                        </SelectItem>
                        <SelectItem value="imaging">Imagerie</SelectItem>
                        <SelectItem value="Examen clinique">
                          Examen clinique
                        </SelectItem>
                      ...
                      {isProcessing ? (
                        ...
                        <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                          ...
                          <p className="text-sm text-gray-600">
                            {
                              Object.values(
                                extractedData.testResultsByCategory
                              ).flat().length
                            }{" "}
                            paramètres détectés
                          </p>
                          {extractedData.processingTime && (
                            <p className="text-xs text-gray-500 mt-1">
                              Temps de traitement:{" "}
                              {extractedData.processingTime.toFixed(2)}s
                            </p>
                          )}
                        </div>
                      ) : (
          ...
          {extractedData && (
            <div className="mt-6">
              ...
              {Object.entries(extractedData.testResultsByCategory).map(
                ([category, results]) => (
                  <div
                    key={category}
